...
class Model(nn.Module):
    def __init__(self, layers, n_inputs, device="cpu", dropout_rate=0.1):
        super().__init__()
        
        self.layers = []
        for nodes in layers:
            self.layers.append(nn.Linear(n_inputs, nodes))
            # self.layers.append(nn.BatchNorm1d(nodes)) # <-- ADD THIS
            # self.layers.append(nn.ReLU())
            self.layers.append(nn.SiLU())
            self.layers.append(nn.Dropout(p=dropout_rate)) # <-- ADD THIS
            n_inputs = nodes
...
class Classifier_CW():
    def __init__(self, 
        ...
        self.optimizer = optim.Adam(self.model.parameters(), lr=learning_rate, weight_decay= 1e-4) # <-- ADD THIS
    ...
    def process_data(self, input_x, input_y, batch_size, weights=None):
        ...
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=60, pin_memory=True)
        val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=60, pin_memory=True)
    ...
    def train(self, input_x, input_y, n_epochs=200, batch_size=512, weights=None, seed=1, outdir="./", early_stop=True, patience=5, min_delta=0.00001, save_model=False, model_name="classifier"):
        ...
        for epoch in tqdm(range(n_epochs), ascii=' >='):
            ...
            for batch_ndx, data in enumerate(train_data):
                ...
                # --- SHAPE ALIGNMENT & WEIGHT NORMALIZATION ---
                # 1. Force outputs and labels to be 1D
                batch_outputs = batch_outputs.squeeze()
                batch_labels = batch_labels.squeeze()

                # 2. Force weights to be 1D and normalize them
                if batch_weights is not None:
                    batch_weights = batch_weights.squeeze()
