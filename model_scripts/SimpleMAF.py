...
#         return torch.tensor(array.astype(np.float32))
    ...
    def process_data(self, data, batch_size, cond=None):
        ...
        train_tensor = self.np_to_torch(data_train).to(self.device)
        val_tensor = self.np_to_torch(data_val).to(self.device)
        
        train_data = torch.utils.data.DataLoader(train_tensor, batch_size=batch_size, shuffle=True, num_workers=0)
        val_data = torch.utils.data.DataLoader(val_tensor, batch_size=batch_size, shuffle=False, num_workers=0)
    ...
    def train(self, data, cond=None, n_epochs=100, batch_size=512, seed=1, outdir="./", early_stop=True, patience=5, min_delta=0.005, save_model=False, model_name="MAF_model", s_b=0):
        ...
        # SPEEDUP: Initialize AMP Scaler
        # New
        scaler = torch.amp.GradScaler('cuda', enabled=(self.device.type == 'cuda'))
        
        update_epochs = 1
        val_loss_to_beat = float('inf')
        
        torch.manual_seed(seed)
        np.random.seed(seed)
        
        epochs, epochs_val, losses, losses_val = [], [], [], []
        ...
        # SPEEDUP: PyTorch 2.0 compiler (optional, uncomment if you use PyTorch >= 2.0)
        # self.flow = torch.compile(self.flow)

        for epoch in tqdm(range(n_epochs), ascii=' >='):
            losses_batch_per_e = []
            self.flow.train() # Ensure model is in train mode
            
            for batch_ndx, batch in enumerate(train_data):
                # Data is already on device if preloaded in process_data
                # SPEEDUP: set_to_none=True is slightly faster than zero_grad()
                self.optimizer.zero_grad(set_to_none=True) 

                if cond is not None:
                    x_ = batch[:, :-self.ncond] if self.nfeat != 1 else batch[:,0].reshape(-1, 1)
                    c_ = batch[:, -self.ncond:] if self.ncond != 1 else batch[:,-1].reshape(-1, 1)
                else:
                    x_ = batch
                    c_ = None
                ...
                # SPEEDUP: Use .item() to prevent GPU pipeline stalling
                losses_batch_per_e.append(loss.item())

            # BUG FIX: Step the scheduler ONCE per epoch, not per batch
            scheduler.step()  
            ...
            if epoch % update_epochs == 0:
                self.flow.eval() # Ensure model is in eval mode for validation
                
                # SPEEDUP: inference_mode is faster and uses less memory than no_grad
                with torch.inference_mode():
                    val_losses_batch_per_e = []
                    for batch_ndx, batch in enumerate(val_data):
                        if cond is not None:
                            x_ = batch[:, :-self.ncond] if self.nfeat != 1 else batch[:,0].reshape(-1, 1)
                            c_ = batch[:, -self.ncond:] if self.ncond != 1 else batch[:,-1].reshape(-1, 1)
                        else:
                        ...
                        val_losses_batch_per_e.append(val_loss.item())
                    ...
                    if mean_val_loss < val_loss_to_beat:
                        val_loss_to_beat = mean_val_loss
                        if save_model:
                            torch.save(self, f"{outdir}/{model_name}.pt")
